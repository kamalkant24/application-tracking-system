...
export const ShortlistedDetails = () => {
    ...
    useEffect(() => {
        ...
        console.log(job_id);
        try {
            fetch(`http://localhost:8000/users/user/${candidate_id}`)
            .then((res) => res.json())
            .then((data) => {
                setCandidate(data)
            })

            fetch(`http://localhost:8000/jobs/current-job/${job_id}`)
            .then((res) => res.json())
            .then((data) => {
                setJob(data)
            })
    ...
    useEffect(() => {
        try {
            fetch(`http://localhost:8000/application/all-application/`)
            .then((res) => res.json())
            .then((data) => {
